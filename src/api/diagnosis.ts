...
export async function startDiagnosis(): Promise<{ historyId: string; diagnosisId: string; status: string }> {
  ...
  const imageKeys = Array.isArray(uploaded) ? uploaded.map((item: any) => item?.key).filter(Boolean) : [];
  if (imageKeys.length === 0) {
    throw new Error("UPLOAD_KEY_MISSING");
  }

  const analysisRes = await apiClient.post("/api/analysis", { imageKeys });
