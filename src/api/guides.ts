...
export type DiyGuide = {
  ...
  steps: string[];
  cautions?: string[];
  materials?: { name: string; approxCost?: string; note?: string }[];
};
...
export async function getDiyGuide(issueType: IssueType): Promise<DiyGuide> {
  ...
  if (issueType === "MOLD") {
    return {
      ...
      materials: [
        { name: "곰팡이 제거제", approxCost: "약 5천~2만원", note: "염소계/비염소계 확인" },
        { name: "마스크/장갑", approxCost: "약 2천~1만원" },
        { name: "걸레/수세미", approxCost: "약 1천~5천원" },
        { name: "제습제/제습기(선택)", approxCost: "상황에 따라" },
      ],
    };
  }
  ...
  if (issueType === "LEAK") {
    return {
      ...
      materials: [
        { name: "방수 테이프/실리콘", approxCost: "약 3천~2만원", note: "응급 처치용" },
        { name: "대야/걸레", approxCost: "약 1천~1만원" },
        { name: "절연 장갑(선택)", approxCost: "약 5천~2만원" },
      ],
    };
  }
...
export async function getExpertInfo(issueType: IssueType): Promise<ExpertInfo> {
  ...
  if (issueType === "MOLD") {
    return {
      ...
      whyPro: [
        "결로/누수 등 근본 원인(구조/설비)이 있으면 DIY로 재발 가능성이 큼",
        "곰팡이가 깊게 침투했으면 벽지/도장/단열 보수가 필요할 수 있음",
        "호흡기 질환/알레르기 있으면 안전 장비와 전문 처리 권장",
      ],
    };
  }
  if (issueType === "LEAK") {
    return {
      ...
      whyPro: [
        "누수는 원인/경로를 찾지 못하면 재발하기 쉬움(탐지 장비 필요)",
        "전기/곰팡이/마감재 손상으로 2차 피해가 커질 수 있음",
        "임대인/세입자 책임 판단에 증빙(견적서/수리내역)이 유리",
      ],
    };
  }
  return {
