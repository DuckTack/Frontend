...
export default function Signup() {
  ...
  const handleCheckUsername = async () => {
    ...
    try {
      ...
      Alert.alert("확인", ok ? "사용 가능한 아이디입니다." : "이미 사용 중인 아이디입니다.");
  ...
  const handleCheckPhone = async () => {
    ...
    try {
      ...
      setPhoneChecked(ok);
      Alert.alert("확인", ok ? "사용 가능한 번호입니다." : "이미 등록된 번호입니다.");
    } catch { Alert.alert("오류", "중복검사 실패"); }
  };
  ...
  const handleCheckEmail = async () => {
    ...
    try {
      ...
      Alert.alert("확인", ok ? "사용 가능한 이메일입니다." : "이미 등록된 이메일입니다.");
    } catch { Alert.alert("오류", "중복검사 실패"); }
  };
  ...
  const handleVerifyEmail = async () => {
    ...
    try {
      ...
      setEmailVerified(ok);
      Alert.alert("이메일 인증", ok ? "인증이 완료되었습니다." : "코드가 일치하지 않습니다.");
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      Alert.alert("가입 실패", "서버 응답을 확인해주세요.");
    } finally {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          <View style={styles.headerTopRow}>
            <View>
              ...
              <Text style={styles.headerSub}>DduckTack의 가족이 되어주세요</Text>
            </View>
            <Pressable onPress={() => router.back()} style={styles.backButtonRight}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.section}>
            ...
            {phoneChecked !== null && (
              <Text style={[styles.statusText, { color: phoneChecked ? "#10b981" : "#ef4444" }]}>
                {phoneChecked ? "✓ 사용 가능한 번호입니다." : "✕ 이미 등록된 번호입니다."}
              </Text>
            )}
          </View>
          ...
          <View style={styles.section}>
            ...
            {emailChecked === true && !emailVerified && (
              <Pressable style={styles.fullWidthBtn} onPress={handleSendEmailCode}>
                <Text style={styles.fullWidthBtnText}>{emailCodeSent ? "코드 재발송" : "인증코드 발송"}</Text>
              </Pressable>
            )}
            {emailCodeSent && !emailVerified && (
              <View style={[styles.row, { marginTop: 10 }]}>
                ...
                <Pressable style={[styles.inlineButton, {backgroundColor: MAIN_BLUE}]} onPress={handleVerifyEmail}>
                  <Text style={[styles.inlineButtonText, {color: '#fff'}]}>확인</Text>
                </Pressable>
              </View>
            )}
          ...
          >
            {isSubmitting ? <ActivityIndicator color="#fff" /> : <Text style={styles.submitButtonText}>회원가입 완료</Text>}
          </Pressable>
...
const styles = StyleSheet.create({
  ...
  header: { paddingHorizontal: 24, paddingTop: 60, paddingBottom: 24, backgroundColor: "#f8faff" },
  headerTopRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' },
  headerTitle: { fontSize: 28, fontWeight: "900", color: "#1e293b" },
  headerSub: { fontSize: 14, color: "#64748b", marginTop: 4 },
  backButtonRight: { width: 44, height: 44, backgroundColor: "#fff", borderRadius: 16, alignItems: "center", justifyContent: "center", elevation: 2, shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 5 },
  ...
  fullWidthBtn: { backgroundColor: MAIN_BLUE, borderRadius: 14, height: 48, alignItems: 'center', justifyContent: 'center', marginTop: 10 },
  ...
  submitButton: { backgroundColor: MAIN_BLUE, height: 62, borderRadius: 20, alignItems: "center", justifyContent: "center", marginTop: 20, marginBottom: 40 },
