...
export default function ReportDetail() {
    ...
    async function persistDraftToServer() {
        if (!reportBase?.diagnosisId) {
            throw new Error("진단 ID가 준비되지 않았습니다.");
        }

        const [beforeUploads, afterUploads] = await Promise.all([
            uploadReportImages(String(reportBase.diagnosisId), draft.beforeImageUris, "BEFORE"),
            uploadReportImages(String(reportBase.diagnosisId), draft.afterImageUris, "AFTER"),
        ]);
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            Alert.alert("생성 요청 완료", "이미지 업로드와 드래프트 저장 후 PDF 생성이 시작되었습니다.");
            await load();
        } catch (error) {
            console.error("report submit failed", error);
            Alert.alert("생성 실패", "이미지 업로드 또는 PDF 생성 요청에 실패했습니다.");
        } finally {
            setSavingDraft(false);
        }
    }
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1, backgroundColor: "#fff" }}>
            <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.actionRow}>
                    <Pressable onPress={handleSubmit} disabled={savingDraft} style={[styles.secondaryButton, savingDraft && { opacity: 0.6 }]}>
                        {savingDraft ? <ActivityIndicator size="small" color="#64748b" /> : <Text style={styles.secondaryButtonText}>PDF 생성 요청</Text>}
                    </Pressable>
