...
import { generateReport, getPdfUrl, type ReportStatus, saveReportDraft } from "../../src/api/reports";
...
export default function ReportDetail() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      // [수정] 묵데이터(Mock) 제거: 실제 API 응답값만 세팅
      setMe(meData);
  ...
  async function handleSaveDraft() {
    if (!reportId || !reportBase?.diagnosisId) return;
    try {
      setSavingDraft(true);
    ...
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1, backgroundColor: "#fff" }}>
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <TextInput value={draft.repairDate} onChangeText={(text) => setDraft((prev) => ({ ...prev, repairDate: text }))} placeholder="수리 완료일 (예: 2026-04-06)" style={styles.input} />

          {draft.repairMethod === "DIY" ? (
            <View style={{ gap: 10 }}>
              <TextInput value={(draft as any).diyMaterialsUsed ?? ""} onChangeText={(text) => setDraft((p) => ({ ...p, diyMaterialsUsed: text } as any))} placeholder="사용한 자재" style={styles.input} />
              <TextInput value={(draft as any).diyMaterialCost ?? draft.materialCost} onChangeText={(text) => setDraft((p) => ({ ...p, diyMaterialCost: text, materialCost: text } as any))} placeholder="자재비" style={styles.input} />
              <TextInput value={(draft as any).diyWorkMemo ?? ""} multiline onChangeText={(text) => setDraft((p) => ({ ...p, diyWorkMemo: text } as any))} placeholder="직접 수리 메모" style={styles.textArea} />
            </View>
          ) : (
            <View style={{ gap: 10 }}>
              <TextInput value={draft.contractorName} onChangeText={(text) => setDraft((p) => ({ ...p, contractorName: text }))} placeholder="업체명" style={styles.input} />
              <TextInput value={draft.contractorContact} onChangeText={(text) => setDraft((p) => ({ ...p, contractorContact: text }))} placeholder="업체 연락처" style={styles.input} />
              
              <TextInput value={draft.materialCost} onChangeText={(t) => setDraft(p => ({ ...p, materialCost: t }))} placeholder="재료비" style={styles.input} />
              <TextInput value={draft.laborCost} onChangeText={(t) => setDraft(p => ({ ...p, laborCost: t }))} placeholder="인건비" style={styles.input} />
              <TextInput value={draft.totalCost} onChangeText={(t) => setDraft(p => ({ ...p, totalCost: t }))} placeholder="총 비용" style={[styles.input, { color: MAIN_BLUE, fontWeight: '700' }]} />
            </View>
          )}

          <TextInput value={draft.repairSummary} multiline onChangeText={(text) => setDraft((p) => ({ ...p, repairSummary: text }))} placeholder="실제 작업 요약" style={[styles.textArea, { marginTop: 10 }]} />
          <TextInput value={draft.notes} multiline onChangeText={(text) => setDraft((p) => ({ ...p, notes: text }))} placeholder="추가 메모" style={[styles.textArea, { marginTop: 10 }]} />
